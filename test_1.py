...
def trajectory(data_type):
    ...
    if data_type != "grid":
        ...
        anim = manimation.FuncAnimation(figure, trajectory_visualisation, interval=100)
        ...
        def surface_simulation(time):
            ...
            j = y_cores[:, 0:time]
            k = z_cores[:, 0:time]
            surface.plot_surface(i, j, k, alpha=0.3, cmap="bone")
            surface.scatter(i, j, k, c=k, cmap="magma", s=0.10)
